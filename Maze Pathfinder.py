...
def find(maze, item):					#finds the grid location of the start and end points
...
def dfs(row, col, visited):				#depth first search
    if maze[row][col] == 'E':		#checks if the endpoint has been found yet 
        return True	#goal found
    visited.add((row,col))				#adds the current grid location to the set visited so it can be checked later
    for move in moves:					#repeats for all 4 possible moves
        ...
        if allowedMove(newRow,newCol):	#checks new location is possible
            if dfs(newRow,newCol,visited):#if the move is possible recursively calls dsf from inside dsf allowing constant repetition until the solution is found
...
#Pros + Cons of Depth First Search
    #PROS:						CONS
    #Fast acting				Doesn't guarantee shortest path
    #Easy to implement			Can cause stack overflow
    #Flexible					Depends on the order of the moves when it is set up
